    ...
    AccessControlUpgradeable
{
    ...
    */
    ...
    function getPoolInfo(
        ...
        view
        checkPid(_pid)
        returns (uint256 minDepositAmount, uint256 unstakeLockedBlocks)
    {
        Pool storage pool_ = pool[_pid];
        minDepositAmount = pool_.minDepositAmount;
        unstakeLockedBlocks = pool_.unstakeLockedBlocks;
        return (minDepositAmount, unstakeLockedBlocks);
    }

    function getPoolWeight(
        uint256 _pid
    ) public view checkPid(_pid) returns (uint256) {
        return pool[_pid].poolWeight;
    }

    function getPoolSTTokenAddress(
        uint256 _pid
    ) public view checkPid(_pid) returns (address) {
        return pool[_pid].stTokenAddress;
    }

    function getPoolSTAmount(
        uint256 _pid
    ) public view checkPid(_pid) returns (uint256) {
        return pool[_pid].stTokenAmount;
    }

    function getUserRequestByIndex(
        uint256 _pid,
        uint256 index
    ) public view checkPid(_pid) returns (uint256, uint256) {
        User storage user_ = user[_pid][msg.sender];
        return (
            user_.requests[index].amount,
            user_.requests[index].unlockBlocks
        );
    }
}
